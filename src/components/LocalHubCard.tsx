...
export const LocalHubCard: React.FC<LocalHubCardProps> = ({ hub, idx, onRedeem }) => {
  ...
  const openDirections = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (typeof hub.latitude === 'number' && typeof hub.longitude === 'number' && !isNaN(hub.latitude) && !isNaN(hub.longitude)) {
  ...
  return (
    <>
      ...
      {typeof hub.latitude === 'number' && typeof hub.longitude === 'number' && !isNaN(hub.latitude) && !isNaN(hub.longitude) && (
